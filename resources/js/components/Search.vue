<template>
    <form class="order-1 pt-2 pb-2 flex-grow-1 d-flex justify-content-center" role="search" style="min-width: 250px"
        id="search">
        <div class="d-flex align-items-center search-icon"><i class="fa-solid fa-magnifying-glass"></i></div>
        <div class="input-group">
            <input @input='getSuggestions' v-model='searchValue' class="form-control" type="search" :placeholder="__('Search') + ' ...'"
                aria-label="Search">
            <button @click="$emit('search', searchValue)" class="btn btn-outline-secondary" type="button">{{ __('Search') }}</button>
        </div>
